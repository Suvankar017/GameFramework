...
**Phase 2 — Core Infrastructure.** Phase 0 laid the structural foundation and Phase 1 built
Bootstrap/Services/Logging/GameState/SceneManagement; Phase 2 adds the infrastructure layer later
systems can safely build on: Time, Timers, Events, Persistence, and Settings. No gameplay systems
(input, audio, UI, progression, ...) exist yet — see [Roadmap](#roadmap).
...
`GameFramework.Runtime` depends only on `GameFramework.Core` — Phase 2 added five new modules
inside that same assembly rather than new ones (see [Assemblies](#assemblies)). Runtime assemblies
must never reference Editor assemblies — no `GameFramework.Editor` assembly exists yet.
...
`GameFramework.Editor` is still not created — no editor-only code exists yet. Phase 2 did not add
any new assembly: all five new modules live inside the existing `GameFramework.Runtime` (and their
tests inside the existing two Runtime test assemblies) — there was no dependency boundary between
them worth a separate assembly for.
...
Phase 2 deliberately does **not** include: Input, Localization, Audio, UI, Progression, Rewards,
Currency, Inventory, Tutorial, Feedback, Pooling, Ads, Analytics, IAP, Remote Config, Feature
Flags, Notifications. Planned next:

- **Phase 3** — Input, Localization, Audio, UI Foundation.
